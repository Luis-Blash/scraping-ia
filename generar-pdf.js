 ...
 *
 * Ejemplos:
 *   node generar-pdf.js lord-of-mysteries-progreso.json
 *
...
function buscarJson() {
  ...
  if (arg) return arg;
  const candidatos = fs.readdirSync('.').filter(f => f.endsWith('-progreso.json'));
  if (candidatos.length === 0) {
    console.error('❌ No encontré ningún *-progreso.json. Corre primero scraper.js');
    process.exit(1);
  }
  return candidatos[0];
}
...
async function main() {
  ...
  const slug = jsonFile.replace(/-progreso\.json$/, '');
  ...
  // Quitar imágenes (el sitio las bloquea con 403). PDF queda solo texto.
